...
export const addMembers: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (!addGroupMembers) {
      throw new HttpError(
        "You are not authorized to add users to this group or the users already exist in the group",
        403
      );
    }
...
export const removeMembers: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (!removeGroupMembers) {
      throw new HttpError(
        "You are not authorized to add users to this group or the group doesn't exist",
        403
      );
    }
