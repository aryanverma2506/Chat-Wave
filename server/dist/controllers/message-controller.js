...
export const getChatMessages = async (req, res, next) => {
    ...
    try {
        ...
        if (!chatInstance) {
            throw new HttpError("Could not found the chat that you are looking for!", 400);
        }
...
export const sendChatMessage = async (req, res, next) => {
    const { chatOrUserId, formattedText: formattedTextStringify, urlPreviewData: urlPreviewDataStringify, } = req.body;
    const senderId = req.userData?.userId;
    try {
        const formattedText = formattedTextStringify && formattedTextStringify !== "undefined"
            ? JSON.parse(formattedTextStringify)
            : undefined;
        const urlPreviewData = urlPreviewDataStringify
            ? JSON.parse(urlPreviewDataStringify)
            : undefined;
        if (!(Array.isArray(formattedText) || urlPreviewData || req.file?.filename) ||
            !chatOrUserId) {
